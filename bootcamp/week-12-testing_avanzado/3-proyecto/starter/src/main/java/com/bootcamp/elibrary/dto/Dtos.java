...
public class Dtos {
  ...
  public record BookCreateRequest(
      ...
      @NotBlank(message = "ISBN is required") @Pattern(regexp = "\\d{13}", message = "ISBN must be 13 digits") String isbn,

      @NotBlank(message = "Author is required") String author,

      @NotNull(message = "Price is required") @DecimalMin(value = "0.01", message = "Price must be positive") BigDecimal price,

      @Min(value = 1, message = "Must have at least 1 copy") int copies) {
  }
  ...
  public record LoanCreateRequest(
      ...
      @Min(value = 1, message = "Minimum loan period is 1 day") @Max(value = 30, message = "Maximum loan period is 30 days") int days) {
  }
