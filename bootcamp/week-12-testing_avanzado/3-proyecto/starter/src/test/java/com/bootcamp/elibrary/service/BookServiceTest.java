...
class BookServiceTest {
    ...
    class FindAll {
        ...
        void returnsPageOfBooks() {
            ...
            // 5. Verificar que los títulos son los correctos usando extracting(BookResponse::title)
        }
    ...
    class Create {
        ...
        void savesBook() {
            // TODO: Implementar
            // 1. Crear un BookCreateRequest con título, isbn "9780132350884", autor, precio 30.0, copies 3
            // 2. Stubbear existsByIsbn → false
            // 3. Stubbear bookRepository.save(any()) → usar thenAnswer para retornar el argumento
            //    con id asignado: thenAnswer(inv -> { Book b = inv.getArgument(0); /* set id via reflection o sub-test */ return b; })
        ...
        void throwsOnDuplicateIsbn() {
            ...
            // 3. Verificar que create() lanza IllegalArgumentException con mensaje sobre ISBN
        ...
        void capturesBookFields() {
            ...
            // 1. Crear BookCreateRequest("Effective Java", "9780134685991", "Joshua Bloch", 49.99, 5)
            // 2. Stubbear existsByIsbn → false
            // 3. Stubbear save() → retornar el argumento (thenAnswer)
            // 4. Ejecutar bookService.create(request)
            // 5. Capturar el Book con bookCaptor: verify(bookRepository).save(bookCaptor.capture())
            // 6. Verificar: título, isbn, autor, precio, totalCopies == 5 del Book capturado
        }
    ...
    class Update {
        ...
        void updatesTitleAndPrice() {
            ...
            // 4. Llamar bookService.update(1L, new BookUpdateRequest("New Title", BigDecimal.TEN))
