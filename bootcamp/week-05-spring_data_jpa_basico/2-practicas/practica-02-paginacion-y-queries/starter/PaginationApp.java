...
interface ProdRepository extends JpaRepository<Prod, Long> {
  ...
  // ============================================
  // @Query("SELECT p FROM Prod p WHERE LOWER(p.name) LIKE LOWER(CONCAT('%',
  // :name, '%'))")
  ...
  // Descomenta este método:
  // ============================================
  // @Query("SELECT p FROM Prod p WHERE p.price <= :maxPrice ORDER BY p.price
  // ASC")
...
class ProdService {
  ...
  // STEP 4: Descomenta:
  // public List<Prod> findByMaxPrice(BigDecimal maxPrice) { return
...
class ProdController {
  ...
  // public Page<Prod> getAll(Pageable pageable) { return
