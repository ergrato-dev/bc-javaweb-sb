...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    /**
     * TODO:
     *  Add a @Query method to search products by name (case-insensitive, partial match).
     *  JPQL: SELECT p FROM Product p WHERE LOWER(p.name) LIKE LOWER(CONCAT('%', :name, '%'))
     *  Returns: List<Product>
     */

    /**
     * TODO:
     *  Add a @Query method to find products with price between minPrice and maxPrice.
     ...
     *  JPQL: UPDATE Product p SET p.stock = p.stock - :qty WHERE p.id = :id AND p.stock >= :qty
