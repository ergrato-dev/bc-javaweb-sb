...
public class OrderService {
    ...
    /**
     * TODO:
     *  1. findByIdWithDetails(id) — load Order with customer + items + products in one query
     ...
     *     b. Check product is active and has sufficient stock, throw IllegalStateException if not
     *     c. Create OrderItem(product, quantity), call order.addItem(item)
     *     d. Decrement product stock via productRepository.decrementStock(productId, qty)
     ...
     * TODO:
     *  1. Load order (throw OrderNotFoundException if not found)
     *  2. Only PENDING or CONFIRMED orders can be cancelled (throw InvalidOrderStatusTransitionException otherwise)
     *  3. Set status to CANCELLED
     *  4. Restore stock for each item: productRepository.decrementStock(-qty) [or add a restoreStock method]
     *  5. Save order
     */
    @Transactional
