...
class DocSimple {
  ...
  public DocSimple(String name, String specialty) {
    this.name = name;
...
class Appt {
  ...
  public Appt(DocSimple doctor, String dateTime, String status) {
    this.doctor = doctor;
    this.scheduledAt = LocalDateTime.parse(dateTime);
    this.status = status;
  }
...
class ApptService {
  ...
  public ApptService(ApptRepo repo) {
    this.repo = repo;
  }
